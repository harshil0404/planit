...
if ( ! class_exists( 'BravePop_MailerLite' ) ) {
   ...
   class BravePop_MailerLite {
      ...
      public function add_to_lists($email, $list_id, $fname='', $lname='', $phone='', $customFields=array(), $tags=array(), $userData=array()){
         ...
         $contact = array('autoresponders'=> true, 'subscribers'=> array( array('email'=> $email , 'fields'=> array('name'=> trim($fullname), 'phone'=> trim($phone) ) ) ));
         
         //Add Custom Field Values
         if(count($customFields) > 0 && $contact['subscribers'][0]['fields']){
            $fieldValues = array();
            foreach ($customFields as $key => $value) {
               $contact['subscribers'][0]['fields'][$key] = $value;
            }
         }
         ...
         if($data && isset($data->errors) && count($data->errors) === 0){
            $userID = '';
            if(isset($data->imported[0]) && isset($data->imported[0]->id)){ $userID = $data->imported[0]->id; }
            if(!$userID && isset($data->updated[0]) && isset($data->updated[0]->id)){ $userID = $data->updated[0]->id; }
            if(!$userID && isset($data->unchanged[0]) && isset($data->unchanged[0]->id)){ $userID = $data->unchanged[0]->id; }
