    ...
    class BravePop_Popup {
        ...
        function __construct($popupID=null, $popupType='popup', $forceLoad=false, $forceStep=false, $customContent=false ) {
               ...
               if($this->popupID && $this->popupData && $popupType === 'popup') {
                  if($this->userTypeMatch && $this->refererMatch && $this->languageMatch && $this->countryMatch && $this->hasCartItems && !in_array(false, $this->cartFilterMatch) && $this->purchaseMatch && $this->notpurchaseMatch) {
      ...
      public function popup_visibility_filter() {
         ...
         if(is_user_logged_in() && !empty($this->popupData->settings->woocommerce->purchase_filter) && function_exists('bravepop_woo_purchase_filter')){
         ...
         if(!empty($this->popupData->settings->filters->countryFilter) && isset($this->popupData->settings->filters->countries) && is_array($this->popupData->settings->filters->countries) && count($this->popupData->settings->filters->countries) > 0){
            if(class_exists('BravePop_Geolocation') && !defined('BRAVEPOP_USER_COUNTRY')){
               ...
               define('BRAVEPOP_USER_COUNTRY',$country_code);
            }            
            ...
            //error_log('User Country: '.$country_code);
         }
      ...
      function popup_render_element($element, $stepIndex, $index, $device, $goalElementIDs ){
         $elements = '';
         switch ($element->type) {
            case 'text':
               ...
               if(isset($stepSettings->content)){
                  foreach ($stepSettings->content as $index => $element) {
                     ...
                     if(isset($element->type) && $element->type !== 'group' && empty($element->group) && (!isset($element->hidden) || $element->hidden === false)){
                        $elements .= $this->popup_render_element($element, $stepIndex, $index, $device, $goalElementIDs );
                     }
                        ...
                        if (class_exists('BravePop_Element_Group')) {
                           ...
                           if(isset($element->elements)){
                              foreach ($element->elements as $chid => $childElmID) {
                                 ...
                                 if($childElement){
                                    $childrenHTML .= $this->popup_render_element($childElement, $stepIndex, $index, $device, $goalElementIDs );
                                 }
                              }
      ...
      public function popup_generate_styles($stepSettings, $index, $device){
         ...
         return $bodyStyle . $popupStyle .$popupMarginStyle . $popupInnerStyle . $popupBackground . $popupBackgroundOverlay . $overlayStyle . $overlayImageStyle . $closeStyle. $popupAnimation. $popupAnimationHover. $popupExitAnimation. $popupOpenAnimation. $elementStyles. $calculatedInerStyles;
